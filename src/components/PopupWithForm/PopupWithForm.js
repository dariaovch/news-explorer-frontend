...
function PopupWithForm({ children, name, title, buttonText, isOpen, onClose, onToggle, onSubmit, isFormValid, mockServerError }) {
    ...
    return (
        <div className={popupClassName}>
            ...
            <form className={`popup__form popup__form_type_${name}`} method="POST" name={`${name}`} noValidate onSubmit={onSubmit}>
                ...
                <button type="submit" className="popup__submit-button" disabled={!isFormValid}>{buttonText}</button>
